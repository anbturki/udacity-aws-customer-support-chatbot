...
# AgentCore attaches managed long-term memory by default, which survives across
# runtimeSessionIds: a fresh chat could recall tickets filed in an earlier one.
# Every chat.py run and every eval case must be independent, so it is disabled.
MEMORY_CONFIG = {"disabled": {}}
# UpdateHarness wraps the same shape in optionalValue so it can be cleared.
MEMORY_CONFIG_UPDATE = {"optionalValue": MEMORY_CONFIG}
